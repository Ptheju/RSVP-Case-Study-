...
SELECT m.year AS year, COUNT(*)
FROM imdb.movie AS m
GROUP BY m.year; 

SELECT Month(m.date_published) AS MONTH_NUM,
...
SELECT m.year, COUNT(DISTINCT id) AS number_of_movies
FROM  imdb.movie as m
...
SELECT distinct genre , COUNT(movie_id) as num_of_movies
FROM imdb.genre
GROUP BY genre
...
with movie_in_one_genre as (
SELECT movie_id
FROM imdb.genre
GROUP BY movie_id
having COUNT(distinct genre) = 1)
select count(*) as movies_with_one_genre
from movie_in_one_genre;
...
FROM imdb.ratings as m
	INNER JOIN imdb.movie as r
		ON m.movie_id = r.id
...
from top_genre_movies as t
inner join imdb.genre as g
on t.genre = g.genre
inner join imdb.movie as m
on g.movie_id = m.id
inner join imdb.director_mapping as d
on m.id = d.movie_id
inner join imdb.names as n
on d.name_id = n.id
inner join imdb.ratings as r
on m.id = r.movie_id
where avg_rating > 8
...
from imdb.movie as m
inner join imdb.role_mapping as ro
on m.id = ro.movie_id
inner join imdb.names as n
on n.id = ro.name_id
inner join imdb.ratings as r
on r.movie_id = m.id
where median_rating > 8 or median_rating = 8
group by name
...
select name, sum(total_votes) as total_votes, count(m.id) as movie_count,
...
group by name
having movie_count = 5 or movie_count > 5
limit 1;
...
group by name
having movie_count = 3 or movie_count > 3
limit 5;
...
(
    ...
    INNER JOIN imdb.genre AS g 
    ON m.id= g.movie_id
	WHERE genre IN (SELECT genre FROM top_genre_movies)
)
...
FROM imdb.names AS n
INNER JOIN imdb.role_mapping AS rm
ON n.id = rm.name_id
INNER JOIN imdb.ratings AS r
